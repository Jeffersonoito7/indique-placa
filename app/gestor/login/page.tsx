...
const NODES = [
  { x: "8%",  y: "15%", s: 3, delay: "0s",   dur: "4s"  },
  { x: "22%", y: "72%", s: 2, delay: "1.2s",  dur: "5s"  },
  { x: "45%", y: "8%",  s: 4, delay: "0.6s",  dur: "3.5s"},
  { x: "68%", y: "55%", s: 2, delay: "2s",    dur: "6s"  },
  { x: "82%", y: "20%", s: 3, delay: "0.3s",  dur: "4.5s"},
  { x: "91%", y: "80%", s: 2, delay: "1.8s",  dur: "5.5s"},
  { x: "35%", y: "88%", s: 3, delay: "0.9s",  dur: "4s"  },
  { x: "58%", y: "35%", s: 2, delay: "2.4s",  dur: "3.8s"},
];

const STREAMS = [
  { left: "12%", delay: "0s",   dur: "7s"  },
  { left: "28%", delay: "1.5s", dur: "9s"  },
  { left: "51%", delay: "3s",   dur: "6s"  },
  { left: "74%", delay: "0.8s", dur: "8s"  },
  { left: "88%", delay: "2.2s", dur: "7.5s"},
];
...
const CSS = `
  ...
  @keyframes borderGlow {
    0%,100% { border-color:rgba(6,182,212,.25); box-shadow:0 0 30px rgba(6,182,212,.06),0 24px 80px rgba(0,0,0,.7); }
    50%      { border-color:rgba(6,182,212,.5);  box-shadow:0 0 50px rgba(6,182,212,.15),0 24px 80px rgba(0,0,0,.7); }
  }
  .lp-root {
  ...
  .lp-stream {
    ...
    background:linear-gradient(to bottom,transparent,rgba(6,182,212,.5),transparent);
    animation:streamFall linear infinite;
    pointer-events:none;
  }
  .lp-card {
    position:relative; width:100%; max-width:390px; text-align:center;
    background:rgba(6,182,212,.04); backdrop-filter:blur(28px); -webkit-backdrop-filter:blur(28px);
    border:1px solid rgba(6,182,212,.25); border-radius:24px;
  ...
  .lp-eye:hover { color:rgba(6,182,212,.8); }
  .lp-btn {
  ...
  .lp-btn:hover:not(:disabled) { opacity:.9; transform:translateY(-1px); box-shadow:0 6px 32px rgba(6,182,212,.55); }
    ...
    <>
      ...
      <div className="lp-root">
        ...
        <div className="lp-card">
          ...
          <div className="lp-badge">Lider / Gestor</div>
          <div className="lp-sub">Acesso do Gestor de Equipe</div>

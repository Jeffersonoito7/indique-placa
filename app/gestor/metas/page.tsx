...
export default function MetasPage() {
  ...
  const criar = async (e: React.FormEvent) => {
    ...
    setSucesso(false);
    try {
      const res = await fetch("/api/gestor/metas", {
        method: "POST",
  ...
  const remover = async (id: string) => {
    ...
    setErro("");
    try {
      const res = await fetch(`/api/gestor/metas?id=${id}`, { method: "DELETE" });
  ...
  return (
    <div className="flex-1 flex flex-col">
      <div className="px-8 py-5 border-b border-border">
        <h1 className="text-base font-bold text-foreground">Metas de Equipe</h1>
        <p className="text-[11px] text-muted-foreground mt-0.5">Defina bonus para seus indicadores e consultores</p>
      </div>

...
export default function WhatsAppPage() {
  ...
  async function desconectar() {
    ...
    setErroWpp(null);
    try {
      const res = await fetch("/api/gestor/whatsapp/disconnect", { method: "POST" });
  ...
  async function salvarConfig() {
    ...
    setErroWpp(null);
    try {
      const res = await fetch("/api/gestor/whatsapp/config", {
        method: "POST",
  ...
  async function iniciarCampanha() {
    ...
    try {
      const res = await fetch("/api/gestor/whatsapp/campanha", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          numeros,
              ...
              <>
                ...
                <Card className="shadow-sm">
                  ...
                  <CardContent className="pt-4 flex gap-3">
                    <button
                      onClick={() => baixarCSV("/api/gestor/exportar/leads", "leads.csv")}
                      className={btnMuted}
                    >
                      Exportar Leads (CSV)
                    </button>
                    <button
                      onClick={() =>
                        baixarCSV("/api/gestor/exportar/indicadores", "indicadores.csv")
                      }
                      className={btnMuted}
                    >

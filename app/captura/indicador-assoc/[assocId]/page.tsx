...
export default function CapturaIndicadorAssocPage({ params }: { params: Promise<{ assocId: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/publico/captura-indicador-assoc/${assocId}`)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setCarregando(true);
    try {
      const res = await fetch(`/api/publico/captura-indicador-assoc/${assocId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nome, telefone, email: email || undefined, senha }),
      });
        ...
        <div className="cap-card">
          ...
          {!linkInvalido && sucesso && (
            <div className="cap-sucesso">
              ...
              <p>Sua conta foi criada. Acesse com seu WhatsApp e senha.</p>
              <a href="/indicador/login" style={{ display: "inline-block", marginTop: 20, color: "#00C389", fontWeight: 700, textDecoration: "none" }}>
                Entrar agora
              </a>
            </div>
          )}
          ...
          {!linkInvalido && !sucesso && (
            <>
              <p className="cap-titulo">Quero ser Indicador</p>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <label className="cap-label">E-mail (opcional)</label>
                <input className="cap-campo" type="email" placeholder="seu@email.com" value={email} onChange={e => setEmail(e.target.value)} />
                ...
                <button className="cap-btn" type="submit" disabled={carregando || !nomeAssoc}>
                  {carregando ? "Cadastrando..." : "Quero indicar e ganhar"}
                </button>
              </form>
            </>
          )}

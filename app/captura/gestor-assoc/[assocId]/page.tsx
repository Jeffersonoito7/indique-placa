...
export default function CapturaGestorAssocPage({ params }: { params: Promise<{ assocId: string }> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setCarregando(true);
    try {
      const res = await fetch(`/api/publico/captura-gestor-assoc/${assocId}`, {
        method: "POST",
        ...
        <div className="cap-card">
          ...
          {linkInvalido && (
            <div style={{ textAlign: "center", color: "rgba(255,255,255,.6)" }}>
              <p style={{ fontSize: 15 }}>Link inválido ou expirado.</p>
            </div>
          )}
          ...
          {!linkInvalido && !sucesso && (
            <>
              <p className="cap-titulo">Cadastro de Gestor</p>
              <p className="cap-sub">
                {nomeAssoc ? `Associação: ${nomeAssoc}` : "Carregando..."}
              </p>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <input className="cap-campo" placeholder="(00) 00000-0000" value={fone} onChange={e => setFone(e.target.value)} required />

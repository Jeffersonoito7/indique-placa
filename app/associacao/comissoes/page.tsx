...
export default function AssociacaoComissoesPage() {
  ...
  const salvar = async (comissao: ComissaoTipo) => {
    ...
    try {
      const res = await fetch("/api/associacao/comissoes", {
        method: "POST",
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 p-8 bg-muted/30">
        <div className="max-w-2xl space-y-4">
          <Card className="shadow-sm border-dashed border-2 border-violet-500/40 bg-violet-500/5">
            ...
            <CardContent className="pt-1 space-y-3">
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-32 px-3 py-2 text-sm rounded-lg border border-border bg-background focus:outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500 transition-all font-mono"
                  />
                </div>
              </div>
              {erroNovo && (
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-violet-600 hover:bg-violet-700 text-white font-semibold text-sm transition-colors disabled:opacity-50"
              >
                <Plus className="h-4 w-4" />
          ...
          {comissoes.map((c) => {
            ...
            return (
              <Card key={c.tipo} className={`shadow-sm border-t-4 ${c.ativo ? "border-t-violet-500" : "border-t-muted"}`}>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-semibold flex items-center justify-between">
                    <span className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="bg-transparent border-0 border-b border-dashed border-border focus:outline-none focus:border-violet-500 text-sm font-semibold w-40"
                      />
                    </span>
                    <div className="flex items-center gap-3">
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        ...
                        <button
                          ...
                          className={`relative w-10 h-5 rounded-full transition-colors ${c.ativo ? "bg-violet-500" : "bg-muted-foreground/30"}`}
                        >
                ...
                <CardContent className="pt-2 space-y-4">
                  {msg && (
                    <div className={`text-xs px-3 py-2 rounded-lg border ${msg.tipo === "ok" ? "bg-violet-500/10 border-violet-500/30 text-violet-600 dark:text-violet-400" : "bg-red-500/10 border-red-500/30 text-red-500"}`}>
                      {msg.texto}
                    </div>
                  )}
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="w-36 px-3 py-2 text-sm rounded-lg border border-border bg-background focus:outline-none focus:ring-2 focus:ring-violet-500/30 focus:border-violet-500 transition-all font-mono"
                      />

...
export async function POST(
  ...
  const { allowed } = await rateLimit(getRateLimitKey(req, "captura-indicador-assoc"), 3, 60 * 1000);
  if (!allowed) return NextResponse.json({ error: "Muitas tentativas. Aguarde 1 minuto." }, { status: 429 });
  ...
  const { data: foneExistente, error: errFone } = await supabaseAdmin
    .from("indicadores").select("id").eq("telefone", fone).maybeSingle();
  ...
  // Buscar consultor_padrao_id das configuracoes, senao indicador fica sem consultor
  const { data: config } = await supabaseAdmin
    .from("configuracoes").select("consultor_padrao_id").maybeSingle();
  ...
  const { error } = await supabaseAdmin.from("indicadores").insert({
    ...
    senha: senha_hash,
    consultor_id: config?.consultor_padrao_id ?? null,
  });

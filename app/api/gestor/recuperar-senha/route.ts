...
export async function POST(req: NextRequest) {
  const { allowed, retryAfter } = await rateLimit(getRateLimitKey(req, "gestor-recuperar-senha"), 5, 15 * 60 * 1000);
  if (!allowed) {
  ...
  if (etapa2.success) {
    ...
    const valido = await validarOTP(tel, "gestor", codigo);
    if (!valido) {
  ...
  const codigo = await criarOTP(tel, "gestor");
  await enviarOTP({ telefone: tel, codigo, tipo: "gestor" });

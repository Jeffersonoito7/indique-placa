...
export async function POST(req: NextRequest) {
  ...
  if (etapa2.success) {
    ...
    const { data: consultor } = await supabaseAdmin.from("consultores").select("id").eq("email", emailNorm).single();
    if (!consultor) { otpStore.delete(emailNorm); return NextResponse.json({ error: "Conta não encontrada" }, { status: 404 }); }
  ...
  const emailNorm = email.toLowerCase();
  const { data: consultor } = await supabaseAdmin.from("consultores").select("nome, email").eq("email", emailNorm).single();
  if (consultor) {
